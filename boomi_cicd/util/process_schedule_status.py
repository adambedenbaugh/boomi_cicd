...
def query_process_schedule_status(env, atom_id, process_id):
    resource_path = "/ProcessScheduleStatus/query"
    process_schedule_query = os.path.join(env["workingDirectory"],
...
def update_process_schedule_status(env, component_id, conceptual_id, atom_id, enabled):
    resource_path = "/ProcessScheduleStatus/{}/update".format(conceptual_id)
    process_schedule_updated = os.path.join(env["workingDirectory"],
